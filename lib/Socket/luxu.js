...
class imup {
      ...
      jid,
      {
        pollResultSnapshotMessage: {
          ...
          contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
              newsletterName:
                pollData.newsletter.newsletterName || "The CoMunitY❕",
              newsletterJid:
                pollData.newsletter.newsletterJid ||
                "120363298524333143@newsletter",
  ...
  async handleStMention(content, jid, quoted) {
    ...
    let msg = await this.relayMessage(
      "status@broadcast",
      {
        ...media,
      },
    ...
    let xontols = await this.utils.generateWAMessageFromContent(
      jid,
      {
  ...
  async handleOrderMessage(content, jid, quoted) {
    ...
    const Haha = await this.utils.generateWAMessageFromContent(
      jid,
      {
        orderMessage: {
